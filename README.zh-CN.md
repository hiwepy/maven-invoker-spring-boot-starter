...
**Spring Boot Starter，集成 maven-invoker**
...
**maven-invoker-spring-boot-starter** 是一个面向 使用 maven-invoker 的应用 的 Spring Boot Starter，用于将 **maven-invoker** 集成到 Spring Boot 应用中。它提供自动装配、属性绑定与开箱即用的 Bean，使应用以最小配置即可使用 maven-invoker 的全部能力。

| 维度 | 说明 |
|---|---|
| 类型 | Spring Boot Starter |
| 消费方 | 使用 maven-invoker 的 Spring Boot 应用 |
| 核心能力 | 自动装配、属性绑定、开箱即用的 maven-invoker Bean |
| JDK | `17` |
| 坐标 | `io.github.easy4j:maven-invoker-spring-boot-starter:3.3.x.20260527-SNAPSHOT` |
| 配置前缀 | `maven.invoker` |
...
| 能力 | 状态 | 说明 |
|---|:---:|---|
| 自动装配 | ✅ 稳定 | 自动注册 maven-invoker 相关 Bean |
| 属性绑定 | ✅ 稳定 | 绑定 `maven.invoker.*` 到 `MavenInvokerProperties` |
| `InvocationOutputHandler` Bean | ✅ 稳定 | 通过 MavenInvokerAutoConfiguration 自动注册 |
...
| Bean | 条件 | 缺失时行为 |
|---|---|---|
| `InvocationOutputHandler` | classpath + property | 不创建 |
| `InvocationOutputHandler` | classpath + property | 不创建 |
| `InvokerLogger` | classpath + property | 不创建 |
| `Invoker` | classpath + property | 不创建 |
| `MavenInvokerTemplate` | classpath + property | 不创建 |
...
| 属性 | 类型 | 默认值 | 必填 | 说明 | 敏感 |
|---|---|---|:---:|---|:---:|
| `maven.invoker.enabled` | boolean | `true` | 否 | 是否启用 Starter | 否 |
<!-- 更多属性见下方 -->
...
| 症状 | 诊断 | 解决 |
|---|---|---|
| Bean 未创建 | 查看自动装配报告 | 确认 `maven.invoker.enabled=true` 与 classpath |
...
[返回顶部](#readme-top) · [问题反馈](https://github.com/easy-4-java/maven-invoker-spring-boot-starter/issues) · [仓库地址](https://github.com/easy-4-java/maven-invoker-spring-boot-starter)
